...
- 성공 흐름은 `session.json`, `prompts.jsonl`, `tools.jsonl`, `errors.jsonl`, `highlights.md`를 포함한 세션 디렉터리를 생성합니다.
- 차단 흐름은 `denied` 도구 이벤트를 기록하고 `highlights.md`에 `Failures Or Denials` 섹션을 추가합니다.
- 실행이 끝나면 `.copilot-highlights/active-sessions.json`은 `{}`로 돌아옵니다.

로컬 개발 중 아직 푸시되지 않은 변경 사항을 테스트하려면 저장소 루트에서 `copilot plugin install ./`로 다시 설치하세요.
